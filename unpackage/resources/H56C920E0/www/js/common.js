var url = 'http://www.zj-zp.com/kitchen/mobile.asmx';
...
//封装上面两个方法 获取最后结果
function getWebservicesreback(data) {
	var data = loadXml(data);
	console.log(data);
	var testJson = xmlToJson(data);
	console.log(testJson.text.text);
	return testJson.text.text;
}

function getWebservicesUrl(functionName) {
	var data = url + '/' + functionName;
	return (data);
}

//调用服务
function goWebservices(methodName, paramTag, callback) {
	console.log(JSON.stringify(paramTag));
	var webservicesUrl = getWebservicesUrl(methodName);
	mui.ajax(webservicesUrl, {
		data: paramTag,
		type: 'POST', //HTTP请求类型
		...
		var  ret= JSON.parse(getWebservicesreback(data));
			callback(ret, textStatus);
			//return true;
...
//			console.log(xhr.response);
		...
		error: function(xhr, type, errorThrown) {
			...
			var  ret= JSON.parse(getWebservicesreback(xhr));
			callback(xhr,type);
			//return false;
		}
	});
